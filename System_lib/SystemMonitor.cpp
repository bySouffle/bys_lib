...
void SystemMonitor::GetDiskPercent(IN const char *path, OUT int *free,
    ...
    //  GB
    *free = static_cast<int>((unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_bfree / 1024 /1024 / 1024) ;
    *total = static_cast<int>( (unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_blocks / 1024 / 1024 /1024);
    *used = *total - *free ;

    *Usage = static_cast<double >( ( *used * 100 * 1.00 ) / (*total) );

    //  kB
#if 0
    df_free = (unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_bfree / 1024 ;
    df_total = (unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_blocks / 1024 ;
    df_usage = ( (df_total - df_free) * 100 * 1.00 ) / (df_total);
#endif
...
int SystemMonitor::GetTotalCpuOccupy(void *param) {
    auto *t = (total_cpu_occupy_t *) param;
...
void SystemMonitor::GetSysInfo(struct utsname* u) {
    uname(u);
}

void SystemMonitor::GetDiskPercent(struct DiskInfo *Idisk) {
    struct statfs path_disk = {};
    if (statfs("/", &path_disk) < 0) {
        perror("path disk can't");
        return;
    }

    //  GB
    Idisk->free = static_cast<int>( (unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_bfree / 1024 /1024 / 1024) ;
    Idisk->total = static_cast<int>( (unsigned long) path_disk.f_bsize * (unsigned long) path_disk.f_blocks / 1024 / 1024 /1024);
    Idisk->used = Idisk->total - Idisk->free ;

    Idisk->Usage = static_cast<double >( ( Idisk->used * 1.00 ) / (Idisk->total) );
